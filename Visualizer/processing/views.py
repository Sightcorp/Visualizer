...
def start_session( request ):
  ...
  if request.method == 'POST':
    ...
    if start_session_form.is_valid():
      new_session = Session.objects.create( start_time = datetime.datetime.now() ) # TODO : Diego : Set client start_time if specified (feature request)
      json_response[ 'code' ] = 0
      json_response[ 'key' ]  = new_session.key
    else:
...
def person_detection( request ):
  ...
  if request.method == 'POST':
    ...
    if person_detection_form.is_valid():
      ...
      json_response[ 'code' ] = 2
      json_response[ 'description' ] = "Invalid request" # TODO : Diego : report error description
